...
function normalizeList(arrOrNull) {
  ...
  if (typeof arrOrNull === "string") {
    ...
    arrOrNull.forEach(item => {
      ...
      if (text.includes(';')) items.push(...text.split(';'));
...
app.storageQueue("GeneratePdfFromQueue", {
  ...
  handler: async (queueItem, context) => {
    context.log(`Processando mensagem da queue (Versão Otimizada no Flow)...`);

    let payload;
    try {
      payload = parseQueueMessage(queueItem) || {};
    } catch (e) {
      context.log.error("Erro ao processar JSON da mensagem");
      throw e;
    }
    ...
    // Construção do ViewModel - As fotos vêm como URLs simples (já reduzidas no Flow)
    const viewModel = {
    ...
    if (!fs.existsSync(templatePath)) throw new Error("Arquivo Preventiva.html não encontrado");
    ...
    // Otimização de PDF para tamanho reduzido
    ...
    try {
      ...
      context.log.error(`Erro Gotenberg: ${err.message}`);
      throw err;
    }
    ...
    context.log(`Sucesso! PDF: ${viewModel.reportId} (${(pdfBuffer.length / 1024).toFixed(2)} KB)`);
  },
});
